...
def get_posts_by_get_param(db: Session = Depends(get_db) , user_id : int = Depends(oauth2.get_current_user)):
...
def get_a_single_post(id : int , response : Response , db: Session = Depends(get_db) , user_id : int = Depends(oauth2.get_current_user)) :
...
def update_post_put(id:int , post : schemas.PostCreate , db: Session = Depends(get_db) , user_id : int = Depends(oauth2.get_current_user)):
    print(user_id)
